...
export function WorkInputForm({ onComplete }: { onComplete?: () => void }) {
  ...
  const handleFreshBagChange = useCallback(
    (data: FreshBagData) => {
  ...
  const handleFreshBagRound1EndChange = useCallback(
    (data: FreshBagData) => {
      updateWorkData(date, {
        freshBag: data,
  ...
  // 미배송 수량 계산 (저장용)
  const undeliveredByRoute = (workData.undelivered || []).reduce(
    (acc, entry) => {
  ...
  const handleSubmit = () => {
    console.info("[input] handleSubmit start");
    // 엑셀식 계산 결과 사용
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="bg-card rounded-2xl p-4 shadow-card border border-border/30">
        <label className="text-xs font-medium text-muted-foreground mb-2 block">날짜</label>
        <div className="flex items-center justify-between">
          <button
            type="button"
      ...
      {/* Stage Indicator */}
      <StageIndicator currentStage={currentStage} onStageChange={handleStageChange} />
